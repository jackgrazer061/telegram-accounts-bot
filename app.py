...
def build_crypto_bulk_result_messages(results, for_whom, max_len=3500):
    ...
    for item in results:
        ...
        if not item.get("octo_ok"):
            error_text = str(item.get("error_text", "")).strip() or "не удалось завести в Octo"
            block_lines.append(f"❌Ошибка: {humanize_issue_error(error_text)}")
...
def process_farm_kings_bulk_proxy_step_background(chat_id, user_id, username):
    ...
    if sync_id:
        try:
            ...
            logging.exception("process_farm_kings_bulk_proxy_step_background basebot sync failed")
        ...
        if str(row[4]).strip().lower() != "free":
            ...
            try:
                ...
                if octo_ok:
                    ...
                    error_text = humanize_issue_error(octo_result)
                    current_item["error_raw"] = str(octo_result)

            except Exception as e:
                logging.exception("process_kings_bulk_proxy_step crashed")
                error_text = humanize_issue_error(e)
                current_item["error_raw"] = str(e)
...
def humanize_issue_error(error_text):
    """Переводит частые технические ошибки выдачи в понятный русский текст."""
    raw = str(error_text or "").strip()
    if not raw:
        return "Octo/бот вернул пустую ошибку. Проверь логи сервера."

    low = raw.lower()

    rules = [
        (
            ["'ellipsis' object has no attribute 'get'", "ellipsis object has no attribute get"],
            "В код вместо данных попала заглушка `...` (ellipsis). Это ошибка в обработке данных/конфига, а не ошибка king. Нужно проверить место, где формируется parsed/proxy/payload."
        ),
        (
            ["cannot access local variable 'parsed' where it is not associated", "local variable 'parsed'"],
            "Бот попытался использовать переменную parsed до того, как она была создана. Обычно это значит, что данные king не разобрались или произошла ошибка до парсинга."
        ),
        (
            ["profile limit", "profiles limit", "limit of profiles", "limit reached", "profile_limit", "profiles_limit", "достиг", "лимит проф"],
            "В Octo достигнут лимит профилей. Удали лишние профили или увеличь лимит в Octo, потом попробуй снова."
        ),
        (
            ["401", "unauthorized", "invalid token", "token"],
            "Octo не принял API-токен. Проверь OCTO_API_TOKEN."
        ),
        (
            ["403", "forbidden"],
            "Octo запретил действие. Проверь права API-токена и доступ к workspace/team."
        ),
        (
            ["429", "rate limit", "too many requests"],
            "Octo временно ограничил запросы (слишком много запросов). Подожди немного и повтори."
        ),
        (
            ["timeout", "timed out", "read timed out", "connect timeout"],
            "Octo или сеть долго не отвечали. Попробуй повторить позже."
        ),
        (
            ["connection", "connectionerror", "failed to establish", "network"],
            "Проблема соединения с Octo/API. Проверь интернет сервера и доступность Octo."
        ),
        (
            ["proxy", "socks5"],
            "Проблема с proxy. Проверь формат, логин/пароль, host/port и что proxy живой."
        ),
        (
            ["user_agent", "user-agent"],
            "Проблема с User-Agent в данных king. Проверь, что User-Agent есть и он начинается с Mozilla/5.0."
        ),
        (
            ["jsondecode", "expecting value", "invalid json"],
            "Бот не смог разобрать JSON. Чаще всего проблема в cookies или ответе API."
        ),
    ]
    ...
    if "octo api error" in low:
        return f"Octo вернул ошибку API.\n\nТехнически: {raw}"
...
def finish_kings_bulk(chat_id, user_id):
    ...
    if not results:
        # Защита от повторного нажатия старых inline-кнопок/дубля callback после завершения выдачи.
        # Раньше из-за этого бот мог уже выдать king, а потом отдельным сообщением написать
        # "Не удалось выдать ни одного king".
        if state.get("mode") == "kings_bulk_done" or not state.get("kings_bulk_queue"):
            send_kings_menu(chat_id, "Выдача king уже завершена или активной очереди нет.")
            return

        clear_state(user_id)
        send_kings_menu(chat_id, "Не удалось выдать ни одного king. Причина: нет результата обработки в очереди.")
        return
    ...
    download_state = {
        ...
        "kings_bulk_queue": state.get("kings_bulk_queue", []),
        "kings_bulk_current_index": len(state.get("kings_bulk_queue", [])),
        "kings_bulk_finished": True,
        "updated_at": time.time(),
        "last_accounts_section": "kings",
    }
    set_state(user_id, download_state)
...
def handle_callback_query(callback_query):
    try:
        ...
        if data == f"kings_bulk_skip_all_proxies:{user_id}":
            state = get_state(user_id)
            if state.get("mode") not in [KING_OCTO_MODE_BULK_CONFIRM, KING_OCTO_MODE_BULK_PROXY] or not state.get("kings_bulk_queue") or state.get("kings_bulk_finished"):
                tg_answer_callback_query(callback_id, "Эта выдача уже завершена")
                return jsonify({"ok": True})
        ...
        if data.startswith("confirm_kings_bulk_octo:"):
            ...
            state = get_state(user_id)
            if state.get("mode") not in [KING_OCTO_MODE_BULK_CONFIRM, KING_OCTO_MODE_BULK_PROXY] or not state.get("kings_bulk_queue"):
                tg_answer_callback_query(callback_id, "Эта выдача уже завершена")
                return jsonify({"ok": True})
